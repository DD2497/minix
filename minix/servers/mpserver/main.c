 ...
 */
  ...
  while (TRUE) {              
      ...
      if (is_notify(callnr)) {
          printf("Mpserver: warning, got illegal notify from: %d\n", m.m_source);
          result = EINVAL;
          goto send_reply;
      }
      switch (callnr) {
      case MPSERVER_SYS1:
          ...
          break;
      default: 
          printf("Mpserver: warning, got illegal request from %d\n", m.m_source);
          result = EINVAL;
      }
...
)
{
    ...
    if (OK != s)
        printf("Mpserver: unable to send reply to %d: %d\n", who_e, s);
}
